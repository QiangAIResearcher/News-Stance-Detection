...
with tf.name_scope('layers'):
    ...
    with tf.name_scope('score_layer'):
        ...
        #s_theta_false = tf.gather(tf.reshape(s_theta, [-1], name='reshape_true'), stances_false_pl, name='s_theta_false')
        ...
        #tf.summary.histogram('score', s_theta)
...
with tf.name_scope('loss'):
    ...
    # l1_loss = tf.contrib.layers.apply_regularization(l1_regularizer, tf_vars)
    l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in tf_vars if 'bias' not in v.name]) * l2_alpha
    rank_loss = tf.maximum(0., 1. - s_theta_true + s_theta_false)
    # rank_loss = tf.square(tf.maximum(0., 1. - s_theta_true + s_theta_false))
    loss = tf.reduce_sum(rank_loss+ l2_loss)# + l2_loss
...
if mode == 'train':
    ...
    with tf.Session() as sess:
        ...
        #train_score, cm = score_submission([LABELS[e] for e in train_stances], [LABELS[e] for e in train_pred])
        ...
        #test_score, cm = score_submission([LABELS[e] for e in test_stances], [LABELS[e] for e in test_pred])
...
#     print("\nloading model...")
#     with tf.Session() as sess:
#         #new_saver = tf.train.import_meta_graph('./model/best-model.meta')
#         #new_saver.restore(sess, tf.train.latest_checkpoint('./model/best-model'))
#         saver = tf.train.Saver()
...
#         test_pred = sess.run(predict, feed_dict=test_feed_dict)
#
# report_score([LABELS[e] for e in train_stances], [LABELS[e] for e in train_pred])
report_score([LABELS[e] for e in test_stances], [LABELS[e] for e in test_pred])
# report_score([LABELS[e] for e in test_stances], ['unrelated']*len(test_stances))
# Save predictions
